...
trait ComponentSpecBase extends PlaySpec with CustomMatchers with GuiceOneServerPerSuite
  ...
  def getBusinessStartDateBeforeLimit(id: String, isEditMode: Boolean, isGlobalEdit: Boolean): WSResponse = {
    get(s"/business/business-start-date-before-limit?id=$id&isEditMode=$isEditMode&isGlobalEdit=$isGlobalEdit")
  }
  ...
  def submitBusinessStartDateBeforeLimit(startDateBeforeLimit: Option[Boolean],
    ...
    post(s"/business/business-start-date-before-limit?id=$id&isEditMode=$isEditMode&isGlobalEdit=$isGlobalEdit")(
