...
case class Address(lines: Seq[String], postcode: Option[String], country: Country, urpn: Option[String] = None) {
  ...
  def encryptedFormat(implicit crypto: Encrypter with Decrypter): OFormat[Address] = {
    ...
    val reads: Reads[Address] = (
      ...
      (lines, postcode, country, urpn) =>
        Address.apply(lines.map(_.decryptedValue), postcode.map(_.decryptedValue), country.getOrElse(UK), urpn.map(_.decryptedValue))
    )
