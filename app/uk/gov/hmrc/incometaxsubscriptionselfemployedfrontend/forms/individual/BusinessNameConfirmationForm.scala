...
object BusinessNameConfirmationForm {

  val fieldName = "yes-no"

  val businessNameConfirmationForm: Form[YesNo] = Form[YesNo](
    single(
      fieldName -> YesNoMapping.yesNoMapping(
        yesNoInvalid = Invalid("error.individual.business-name-confirmation.empty")
      )
    )
  )
