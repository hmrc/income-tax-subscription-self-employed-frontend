...
object DateModelMapping {
  ...
  def dateModelFormatter(isAgent: Boolean = false): Formatter[DateModel] = new Formatter[DateModel] {

    def errorKey(error: String): String = if (isAgent) s"error.agent.$error.empty" else s"error.$error.empty"
  ...
  def dateModelMapping(isAgent: Boolean = false): Mapping[DateModel] = of[DateModel](dateModelFormatter(isAgent = isAgent))
