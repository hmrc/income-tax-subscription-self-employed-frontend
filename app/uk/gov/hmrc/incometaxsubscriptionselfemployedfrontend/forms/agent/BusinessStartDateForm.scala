...
object BusinessStartDateForm {
  ...
  def businessStartDateForm(minStartDate: LocalDate, maxStartDate: LocalDate, f: LocalDate => String): Form[BusinessStartDate] = Form(
    mapping(
      startDate -> DateModelMapping.dateModelMapping(isAgent = true, errorContext = errorContext, minDate = Some(minStartDate), maxDate = Some(maxStartDate), dateFormatter = Some(f))
