...
package uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.auth
...
import uk.gov.hmrc.auth.core.{AuthConnector, AuthorisationException, InvalidBearerToken}
...
trait MockAuth extends MockitoSugar with BeforeAndAfterEach {
  self: Suite =>
    ...
    def mockAuthSuccess(): Unit = {
      when(mockAuth.authorise(
          ArgumentMatchers.any[Predicate],
          ArgumentMatchers.any[Retrieval[Unit]]
