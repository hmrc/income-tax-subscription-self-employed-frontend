...
class AgentMainTemplateViewSpec extends ViewSpec {

  implicit val request: FakeRequest[AnyContentAsEmpty.type] = fakeTestRequest

  object AgentMainTemplateMessages {
    val title = "testTitle"
    val titleSuffix = " - Use software to report your client’s Income Tax - GOV.UK"
  }

  def page(enabledTimeout: Boolean): HtmlFormat.Appendable = views.html.templates.agent_main_template(
    title = AgentMainTemplateMessages.title,
  ...
  "The Agent Main Template view" should {
    "display the title" in new Setup {
      doc.title mustBe AgentMainTemplateMessages.title + AgentMainTemplateMessages.titleSuffix
