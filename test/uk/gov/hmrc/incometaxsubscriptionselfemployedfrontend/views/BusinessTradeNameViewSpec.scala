...
class BusinessTradeNameViewSpec extends ViewSpec {
  ...
  class Setup(businessTradeNameForm: Form[BusinessTradeNameModel] = BusinessTradeNameForm.businessTradeNameValidationForm) {
  ...
  "Business Trade Name" must {
    ...
    "must display form error on page along with textInput and hintText" in
      new Setup(BusinessTradeNameForm.businessTradeNameValidationForm.withError(testError)) {
