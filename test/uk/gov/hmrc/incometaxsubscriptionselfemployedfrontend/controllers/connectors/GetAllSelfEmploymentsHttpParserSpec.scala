...
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.controllers.utilities.TestModels._
  ...
  "GetAllSelfEmploymentHttpReads" when {
    "read" should {
      "parse a correctly formatted OK response as a GetAllSelfEmploymentsDataModel" in {
        val httpResponse = HttpResponse(OK, testGetAllSelfEmploymentModelJsValue)
      ...
      "parse an no content response as None" in {
        val httpResponse = HttpResponse(NO_CONTENT, Json.obj())
