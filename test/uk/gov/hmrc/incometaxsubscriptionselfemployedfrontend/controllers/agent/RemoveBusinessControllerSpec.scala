...
class RemoveBusinessControllerSpec extends ControllerBaseSpec with MockIncomeTaxSubscriptionConnector {
  ...
  "show" should {
    ...
    "return (303) redirect to Initialise Controller when Remove business has one business and click remove business" when {
      "the connector returns successful json" in {
        mockAuthSuccess()
        mockGetSelfEmployments[Seq[SelfEmploymentData]]("Businesses")(Right(Some(Seq(
          businessData
    ...
    "return (303) redirect to business start date that is Initialise controller page" when {
