...
class UrlHelpersSpec extends AnyFunSuite with UrlHelpers {
  ...
  test("can infer agent from source path") {
    ...
    val actual = redirectUrlOf(baseUrl, "/p/q/client/r/s")
    val expected = "http://a/b/c/client"
    assert(actual === expected)
  }

  test("can handle non-agent source path") {
    val baseUrl = "http://a/b/c"
    val actual = redirectUrlOf(baseUrl, "/p/q/r/s")
