...
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.connectors.httpparser.SaveSessionDataHttpParser._

class SaveSessionDataHttpParserSpec extends PlaySpec {
  ...
  "SaveSessionDataHttpParser" when {
    "read" should {
      "parse a correctly formatted OK response and return the data in a model" in {
        val httpResponse = HttpResponse(status = OK, body = "")
        val res = saveSessionDataHttpReads.read(testHttpVerb, testUri, httpResponse)

        res mustBe Right(SaveSessionDataSuccessResponse)
      }
