...
class GetAllSelfEmployedDetailsHttpParserSpec extends UnitTestTrait {
  ...
  "GetSelfEmploymentHttpReads" when {
    "read" should {
      "parse a correctly formatted OK response and return the data in a model" in {
        ...
        val res = getAllSelfEmployedDetailsHttpReads[DummyModel].read(testHttpVerb, testUri, httpResponse)
      ...
      "parse an incorrectly formatted Ok response as an invalid Json" in {
        ...
        val res = getAllSelfEmployedDetailsHttpReads.read(testHttpVerb, testUri, httpResponse)
