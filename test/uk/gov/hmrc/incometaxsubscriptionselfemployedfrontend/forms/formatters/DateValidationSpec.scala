...
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.forms.formatters.DateModelMapping.HtmlIds
...
class DateValidationSpec extends AnyWordSpec with Matchers {
  ...
  "DateField" should {
    "reorder in alphabetical order" in {
      List(YearField, MonthField, DayField).sorted mustBe List(DayField,  MonthField, YearField)
    }
  }
