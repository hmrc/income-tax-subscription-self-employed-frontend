...
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.forms.formatters.DateErrorMapping._
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.forms.formatters.DateModelMapping.{DateModelValidation, HtmlIds}
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.forms.formatters.DateValidation.{DayField, MonthField, YearField}
  ...
  "highlightField" should {
    ...
    "return 'false'" when {
      ...
      "the month field key not is in the message key" in {
        DateErrorMapping.highlightField(MonthField, "agent.error.monthly-update.day.empty") mustBe false
      }

      "the year field key not is in the message key" in {
        DateErrorMapping.highlightField(YearField, "agent.error.financial-year.month.empty") mustBe false
      }
    }
  }
