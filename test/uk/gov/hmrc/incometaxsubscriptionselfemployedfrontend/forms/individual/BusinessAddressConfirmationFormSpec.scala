...
class BusinessAddressConfirmationFormSpec extends PlaySpec {

  "businessAddressConfirmationForm" must {
    "bind successfully into Yes" when {
      "bound with the correct form key and a yes value" in {
        val boundForm = businessAddressConfirmationForm.bind(Map(fieldName -> YesNoMapping.option_yes))
    ...
    "bind successfully into No" when {
      "bound with the correct form key and a no value" in {
        val boundForm = businessAddressConfirmationForm.bind(Map(fieldName -> YesNoMapping.option_no))
        boundForm.hasErrors mustBe false
    ...
    "produce a form error" when {
      "there was no data provided to the binding form" in {
        val emptyForm = businessAddressConfirmationForm.bind(Map.empty[String, String])
        emptyForm.value mustBe None
        emptyForm.errors.headOption mustBe Some(FormError(fieldName, "error.individual.business-address-confirmation.empty"))
      }
      "there was an invalid value provided to the binding form" in {
        val invalidForm = businessAddressConfirmationForm.bind(Map(fieldName -> "invalid"))
        invalidForm.value mustBe None
        invalidForm.errors.headOption mustBe Some(FormError(fieldName, "error.individual.business-address-confirmation.empty"))
      }
    }
  }
