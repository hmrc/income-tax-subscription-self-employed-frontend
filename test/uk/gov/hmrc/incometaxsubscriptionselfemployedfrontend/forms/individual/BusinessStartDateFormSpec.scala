...
import uk.gov.hmrc.incometaxsubscriptionselfemployedfrontend.forms.individual.BusinessStartDateForm.{businessStartDateForm, featureSwitchErrorContext, startDate}
  ...
  "The BusinessStartDateForm" when {
    "the start date before limit feature switch is disabled" should {
      def form: Form[DateModel] = {
        businessStartDateForm(BusinessStartDateForm.minStartDate, BusinessStartDateForm.maxStartDate, d => d.toString, startDateFeatureSwitchEnabled = false)
      }
    ...
    "the start date before limit feature switch is enabled" should {
      def form: Form[DateModel] = {
        businessStartDateForm(BusinessStartDateForm.minStartDate, BusinessStartDateForm.maxStartDate, d => d.toString, startDateFeatureSwitchEnabled = true)
      }
      ...
      "when testing the validation" should {
        ...
        "output the appropriate error messages for the start date" when {
          ...
          "it is before 1900" in {
            val minTest = form.bind(DataMap.date(startDate)("31", "12", "1899"))
            minTest.errors must contain(FormError(dayKeyError, beforeMin, Seq(BusinessStartDateForm.minStartDateLimit.toString)))
          }
